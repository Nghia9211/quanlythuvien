...
class Tx implements CirculationTransaction {
  ...
  async findLoanById(id: string) { const row = await this.m.getRepository(LoanOrmEntity).createQueryBuilder("loan").leftJoinAndSelect("loan.items", "item").where("loan.id = :id", { id }).setLock("pessimistic_write").getOne(); return row ? this.loan(row) : null; }
